...
export interface MemoryNamespaceFace {
  ...
  setSettings(settings: { settings: MemorySettings }): Promise<RemoteResult<{ ok: boolean }>>
  readProfile(): Promise<RemoteResult<{ content: string }>>
  writeProfile(content: { content: string }): Promise<RemoteResult<{ ok: boolean }>>
  writeProfileFact(args: { section: string; fact: string }): Promise<RemoteResult<{ ok: boolean }>>
  listTopics(): Promise<RemoteResult<{ topics: string[] }>>
  readTopic(topic: { topic: string }): Promise<RemoteResult<{ content: string }>>
  search(keywords: { keywords: string }): Promise<RemoteResult<{ items: MemoryItem[] }>>
