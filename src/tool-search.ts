...
/** Cache a short-lived copy so the per-step section provider stays cheap. */
const CACHE_MS = 5000

/**
 * M3: inject a bounded memory context into the model prompt. Registers one
 * system-prompt section evaluated on every assembly (per step), so contents
 * stay fresh but reads are cached to the latest assemble only.
 */
