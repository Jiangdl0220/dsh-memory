    ...
    services: [
      {
        ...
        members: [
          ...
          { kind: 'method', name: 'setSettings', signature: 'setSettings(request: { settings: MemorySettings }): Promise<{ ok: boolean }>' },
          { kind: 'method', name: 'readProfile', signature: 'readProfile(): Promise<{ content: string }>' },
          { kind: 'method', name: 'writeProfile', signature: 'writeProfile(request: { content: string }): Promise<{ ok: boolean }>' },
          { kind: 'method', name: 'writeProfileFact', signature: 'writeProfileFact(request: { section: string; fact: string }): Promise<{ ok: boolean }>' },
          { kind: 'method', name: 'listTopics', signature: 'listTopics(): Promise<{ topics: string[] }>' },
          { kind: 'method', name: 'readTopic', signature: 'readTopic(request: { topic: string }): Promise<{ content: string }>' },
          { kind: 'method', name: 'search', signature: 'search(request: { keywords: string }): Promise<{ items: MemoryItem[] }>' },
